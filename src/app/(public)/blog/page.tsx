    ...
    <div className="bg-[#F7EBDD] min-h-screen text-[#131312] relative overflow-hidden">
      ...
      <div className="section-container py-16 relative z-10">
        {posts.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 lg:gap-8 pb-24">
            {posts.map((post) => (
              <div 
                key={post._id} 
                className="group flex flex-col transition-all duration-300 h-[380px] lg:h-[420px] bg-white border border-black/10 p-4 relative hover:border-[#D85A30]/40 shadow-sm"
              >
                {/* Blog Image Area */}
                <div className="relative h-[60%] bg-[#FCF6ED] overflow-hidden">
                  {post.featuredImage ? (
                    <Image
                    ...
                    <div className="absolute inset-0 flex items-center justify-center text-xs text-muted-foreground bg-[#FCF6ED]">
                      No Image Available
                    </div>
                  )}
                </div>
                {/* Blog Info Area */}
                <div className="h-[40%] pt-4 flex flex-col justify-between">
                  <div className="space-y-2">
                    <div className="flex items-center gap-3 text-[10px] text-muted-foreground uppercase font-sans font-semibold">
                      <span className="flex items-center gap-1">
                        <Calendar className="h-3 w-3 text-[#D85A30]" />
                        {new Date(post.publishedAt).toLocaleDateString()}
                      </span>
                    </div>
                    <h3 className="font-heading text-sm font-bold text-[#131312] group-hover:text-[#D85A30] transition-colors line-clamp-2 leading-snug">
                      {post.title}
                    </h3>
                  </div>
                  <Link
                    href={`/blog/${post.slug}`}
                    className="label-tech !text-[#D85A30] flex items-center gap-1 mt-auto group-hover:underline"
                  >
                    Read Full Article
